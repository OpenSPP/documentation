...
The [spp_entitlement_in_kind](spp_entitlement_in_kind) module enhances the existing **[spp_programs](spp_programs)  (Programs)** module by introducing specific features and workflows for handling in-kind entitlements, where beneficiaries receive goods or services instead of cash transfers. 
...
1. **In-Kind Entitlement Definition:**
    * Extends the `spp.entitlement.inkind` model from [spp_programs](spp_programs) to specifically manage in-kind entitlements.
...
2. **Entitlement Manager Extension:**
    * Inherits from the `spp.program.entitlement.manager` model in [spp_programs](spp_programs).
    * Introduces a new type of entitlement manager (`spp.program.entitlement.manager.inkind`) designed for in-kind distributions. 
