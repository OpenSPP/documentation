...
1. **OpenSPP Registry (`spp_registry`):**  This module indirectly depends on the `res.partner` model from the OpenSPP Registry module, as it modifies the access rules for registrant data based on a user's assigned areas.

2. **OpenSPP Registry (`spp_registry`):**  Similar to the Base module, it impacts access to group registrant data based on area assignments.
