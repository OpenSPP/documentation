...
The `spp_farmer_registry` module serves as the foundation for managing farmer registries within the OpenSPP system. It builds upon several core OpenSPP modules to provide functionalities specific to farmer registration and farm data management. 
...
1. **spp_registry**: Used to manage memberships between individual registrants and groups.
...
4. **spp_registry**: Provides the basic structure for managing registrant data, relationships, and identification.
5. **spp_registry**: Extends the base registry to handle groups of registrants, such as farmer cooperatives.
6. **spp_registry**: Extends the base registry with functionalities for managing individual registrant data.

## Functionality and Integration Details

## Farmer Information

* Extends the `res.partner` model (from **spp_registry**) to include farmer-specific details like years of experience, formal agricultural training, household size, etc.
...
* Integrates with **spp_registry** to leverage existing features for managing individual registrant profiles.
